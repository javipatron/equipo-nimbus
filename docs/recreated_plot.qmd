...
```{r}
pivot_df_no3_k_rejoined <- convert_monthly_to_bimonthly(df = pivot_df_no3_k_rejoined, month_column = month_date) 
```
...
recreated_plot <- ggplot(data = summary_df, aes(x = odd_month_date, 
  ...
  labs(title = "Bimonthly Chemical Means by Year", x = "Years", y = "Bimonthly Chemical Means", color = "") + # add something about puerto rico to title
  theme_light() +
